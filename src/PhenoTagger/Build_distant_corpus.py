...
def train_pos(hpo_obo,hpo_vocab,outpath):
    ...
    for hpoid in hpo_vocab:
        if hpoid=='HP:None' or hpoid=='MS:None':
            continue
...
def train_neg(negfile,num,hpo_dic,outpath,MS=True):
...
#        print(negative_text)
        ...
        if negative_text not in hpo_dic:         
            if not MS:
                ...
                temp_out.append('MS:None\t'+negative_text)
            token_pos = nltk.pos_tag(tokens)
            for token in token_pos:
                lemma = lemmatizer.lemmatize(token[0], get_wordnet_pos(token[1])) 
                stem = stemmer.stem(token[0])                
...
if __name__=="__main__":
    ...
    parser.add_argument('--fileneg', '-f', help="the text file used to generate the negatives",default='../../../data/train_data/raw_articles.txt')
    parser.add_argument('--negnum', '-n', help="the number of negatives ",type=int, default=60000)
    parser.add_argument('--output', '-o', help="the output folder of the distantly-supervised training dataset",default='../../../data/train_data/distant_train_data/')
